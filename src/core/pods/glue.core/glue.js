var Structr = require('structr'),
	...
	ProxyRequest = require('brazln').ProxyRequest,
...
var GlueConnection = Structr({  
	...
	{    
		...
		this._proxy.pull('channels', function(channels)
		{
			function listenOnPush(channel)
			{
				self._proxy.on('push -stream ' + channel, function(stream)
				{ 
					if(!self.ignore(channel))
					{
						self.push(channel, function(writer)
						{ 
							stream.pipe(writer);
						});
					}
			...
			{
				listenOnPush(channels[i]);
			}
		});


		//get all the channels so we can listen for them
		self.pull('channels', function(channels)
		{             
			self._reset();     
			...
			{   
				...
				self._proxy.on('pull -stream ' + channel, function(writer)
				{               
					if(!self.ignore(channel))
					{
						self.pull('-stream ' + channel, function(reader)
						{
							reader.pipe(writer);
						}); 
					}
	...
	'_onPull': function(channel)
	{
		...
		this._remoteProxy.on('pull -stream ' + channel, function(writer)
		{
	...
	'_onPush': function(channel)
	{
		...
		self._remoteProxy.on('push -stream ' + channel, function(reader)
		{
