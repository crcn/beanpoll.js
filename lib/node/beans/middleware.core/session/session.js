...
var Session = Structr({
	...
	'__construct': function(ops,headers)
	{
		...
		//needed so we can keep track of the browser, and other stuff
	...
	'toString': function()
	{
		...
		return Cookie.getString('SID',this.id);
	}
});


var SessionManager = Structr({
	'getSession': function(request,callback)
	{
		var headers = request.client.headers,
		
			//le cookie
			cookie = headers.cookie || '',
			
			//split apart the cookie so we only have the sessid
			sessidParts = cookie.match(/SID=([^;]+)/),
			
			//is a session is passed, we need to check if the browser's the same as the one stored with the sessid
			key = request.client.headers['user-agent'],
			self = this;
		...
		if(sessidParts)
		{
			var id = sessidParts[1];

			this.getData(id,key,function(data)
			{
