...
{ 
 	...
 	mw.add(function(ops, next)
 	{
		...
		fs.stat(fullPath, function(err,stat)
		{
			...
			res.writeHead(200, { 'Content-Length': stat.size, 'Content-Type': ops.mime } );
			

			fs.createReadStream(fullPath, {flags: 'r', 
			encoding: 'binary', 
			mode: 0666,
			bufferSize: 4 * 1024})
	...
	function onRequest(req, res)
	{
		...
		mw.request(ops, function(newOps)
		{
			...
			router.pull(newOps.channel, newOps.query, { req: req, meta: newOps.requestMeta, host: newOps.host }, function(request)
			{
				...
				this.pipe({
					...
					write: function(data)
					{
						//is the data an object? is pretty provided? 
						var chunk = data instanceof Object ? JSON.stringify(data) : data;
