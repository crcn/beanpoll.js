...
{ 
 	...
 	function sendFile(path, res)
 	{
		fs.stat(path, function(err, stat)
		{
			...
			res.writeHead(200, { 'Content-Length': stat.size, 'Content-Type': ops.mime || mime.lookup(path) } );
	...
	function onRequest(req, res)
	{
		...
		mw.request(ops, function(newOps)
		{
			...
			router.pull(newOps.channel, newOps.query, { req: req, meta: newOps.requestMeta, host: newOps.host, sendFile: sendFile }, function(request)
			{
