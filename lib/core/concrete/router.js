...
var Router = Structr({
	...
	'on': function(expr, ops, callback)
	{
		...
		for(var i = expr.channels.length; i--;)
		{
			...
			{         
				...
				if(single.channel.paths[single.channel.paths.length-1].param == epath[epath.length-1].param && !expr.meta.before && !expr.meta.after)
				{                                            
	...
	'getRoute': function(single, data, middleware)
	{
		...
		if(middleware) 
		{
			r.listeners = this._filterRoute(single, this._collection.middleware(single.channel)).concat(r.listeners);
		}
