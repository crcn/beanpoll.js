...
var Router = Structr({
	...
	'on': function(expr, callback)
	{
		for(var i = expr.channels.length; i--;)
		{
			...
			if(this.hasRoute(single) && !this._allowMultiple  && !this._middleware().allowMultiple(single))
			{
	...
	'getRoute': function(single, data)
	{
		...
		return this._middleware().getRoute({
			...
			route: route.target,
			data: data,
			listeners: this._filterRoute(single, route.target)
		});
	},
	...
	{	
		...
		return this._dispatch(utils.channel(expr,0), data, ops, callback);
	},
	...
	'_dispatch': function(expr, data, ops, callback)
	{
		...
		{
			if(!ops.ignoreWarning && !expr.meta.passive) console.warn('The %s route "%s" does not exist', expr.type, utils.pathToString(expr.channel.paths));
