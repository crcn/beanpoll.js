...
var Request = Structr({
	...
	'next': function(data)
	{
		...
		if(!this._calledListeners)
		{
			...
			if(!this.router._allowMultiple && route._meta && route._meta.rotate != undefined && listeners.length)
			{
	...
	'_passThru': function(ops, allThru)
	{
		...
		if(allThru.length)
		{
			...
			return this.router.dispatch(thru, this.data, function()
			{	
...
var Router = Structr({
	...
	'hasRoute': function(expr)
	{
		return !!this.getRoute(expr).listeners.length;
	},
	...
	'getRoute': function(expr)
	{
		...
		return { route: currentRoute, thru: thru, path: paths, listeners: this._filterRoute(expr, currentRoute) }	
	},
