...
var Request = Structr({
	...
	'next': function(data)
	{
		...
		if(!this._calledListeners)
		{
			...
			//if rotate is specified, then we need to rotate it (round-robin). There's a trick catch though...
			//because the above *might* have filtered down to metadata values, we need to only rotate what's left, AND
			//the rotate index must be stuck with routes rotate metadata
			if(route._meta.rotate != undefined)
			{
