	...
	'next': function(data)
	{
		...
		if(!this._calledListeners)
		{
			...
			var route = this.route,
				loadBalance = route._loadBalance;


			if(loadBalance)
			{
				//round-robin this shit.
				var callback = route._callbacks.pop();
				route._callbacks.unshift(callback);
			}
...
var Router = Structr({
	...
	'on': function(expr, callback)
	{
		...
		if((currentRoute._callbacks.length || currentRoute._thru.length) && !this._allowMultiple && !expr.meta.rotate)
		{
	...
	{	
		...
		if(i != n && !this._ops.ignoreWarning)
		{
