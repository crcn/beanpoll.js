...
var ChannelParser = function()
{
	...
	this.parse = function(source)
	{
		...
		if(cache[source]) return cache[source];//Structr.copy(cache[source]);
		
		tokenizer.source(source);

		return cache[source] = rootExpr();//Structr.copy(cache[source] = rootExpr());
	}
	...
	var rootExpr = function()
	{
		...
		if(tokenizer.isWhite(tokenizer.peekChars(1).charCodeAt(0)) && tokenizer.position() < tokenizer.source())
		{
			type = expr.value;
			tokenizer.next();
		}
...
console.log(exports.parse('name'));
