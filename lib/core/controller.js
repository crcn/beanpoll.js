...
var Controller = Structr({
	...
	'__construct': function()
	{
		...
		this._routeMiddleware = routeMiddleware(this);
		this._channels = {};

		var types = this._routeMiddleware.types;
		
		for(var i = types.length; i--;)
		{
			this._createTypeMethod(types[i]);
		}
	},
	...
	{
		var expr = this._parse(type, ops),
			router = this._routeMiddleware.router(expr),
	...
	'_createTypeMethod': function(method)
	{
		...
		this[ method ] = function(type, data, ops, callback)
		{
			...
			this._routeMiddleware.router(expr).dispatch(expr, data, ops, callback);
		}
	}
