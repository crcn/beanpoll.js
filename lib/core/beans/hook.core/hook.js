...
var HookConnection = Structr({  
	...
	{                          
		...
		if(msg.action == 'pull' || msg.action == 'push')
		{
			...
			if( true /*router.grantAccess(msg.name, (msg.data || {}).pullKey)*/)
			{
				if(msg.action == 'pull')
				{
					self._router.pull(msg.name, msg.data, { meta: { stream: 1}, from: self._remoteRouter }, function(reader)
					{                 
						reader.pipe({
	...
	{    
		...
		{
			self._janitor.addDisposable(self._router.on(channel, ops, function(stream)
			{ 
				if(!self.ignore(channel))
				{
					self[type].call(self, channel, stream.data, { meta : {stream: 1} }, function(writer)
					{ 
						if(type == 'pull')
						{
							writer.pipe(stream);
		...
		function listenToChannel(channel, hook)
		{
			...
			var type = channel.name.split(' ').shift();
			...
			self._on(type, channel.name);
			listenOn(type, ops, channel.name);
	...
	{
		var self = this;
		this._janitor.addDisposable(this._remoteRouter.on(channel, { type: 'pull', meta: {stream:1}}, function(writer)
		{
			console.log(this.hasNext());
			
			self._request('pull', utils.pathToString(this.current.path), writer);
		}));
	},
	...
	'_onPush': function(channel)
	{
		...
		this._janitor.addDisposable(self._remoteRouter.on(channel, { type: 'push', meta: {stream:1}}, function(reader)
		{
			var ch = utils.pathToString(this.current.path);

			var uid = self._request('push', ch);
