...
var LocalInvoker = Structr({
	...
	'hook': function(type, channel, ops)
	{
		...
		this._janitor.addDisposable(this._router.on(type + ' ' + channel, ops, function(localRequest)
		{
			...
			if(this.inner.isRemote && (!self._con._target.bussed || !this.inner.key || !self._con._remote.id || self._con._remote.id == this.inner.key)) return;
	...
	'pull': function(trans)
	{
		...
		this._router.pull(trans._name, trans._data.data, { from: this._con._remote, meta: this._meta(trans, 'pull'), _next: this._next(trans), inner: this._inner(trans)  }, function()
		{
			this.pipe({
