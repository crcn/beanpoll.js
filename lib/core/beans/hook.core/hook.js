...
var RemoteInvoker = Structr({
	...
	'__construct': function(connection)
	{
		...
		this._connection._router.on('push -pull hook', { data: { all: connection._target.bussed } }, function(hook)
		{
	 ...
	 */
	 ...
	 '_hookPull': function(channel)
	 {
	 	...
	 	this._janitor.addDisposable(this._virtualRouter.on('pull -stream ' + channel, function(request)
	 	{
	 		...
	 		if(!this.inner.key) this.inner.key = self._hook.id;

	 		self._connection._transaction(this.channel).
	 		send('pull', { hasNext: this.hasNext(), data: this.data, inner: this.inner } ).
	 		on(this).
	 		on({
	 			next: this.next,
	 		}).disposeOn('end');
...
var LocalInvoker = Structr({
	...
	'hook': function(type, channel, ops)
	{
		...
		{
			// console.log(self._con._remote.id+" "+this.inner.key)
			...
			remoteMethod(this.channel, localRequest.data, { meta: { stream: true }, _next: _next, inner: this.inner }, function(remoteRequest)
			{
				if(type == 'pull')
				{
	...
	'push': function(trans)
	{
		...
		this._router.push(' -stream ' + trans._name, {}, { from: this._con._remote, meta: this._meta(trans, 'push'), _next: this._next(trans), inner: this._inner(trans) }, function(stream)
		{
	...
	'pull': function(trans)
	{
		...
		this._router.pull(' -stream ' + trans._name, trans._data.data, { from: this._con._remote, meta: this._meta(trans, 'pull'), _next: this._next(trans), inner: this._inner(trans)  }, function()
		{
			this.pipe({
