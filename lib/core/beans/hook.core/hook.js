...
var RemoteInvoker = Structr({
	...
	'__construct': function(hook)
	{
		...
		this._hook._router.on('push -pull hook', { data: { all: hook._target.allHooks } }, function(hook)
		{
	 ...
	 */
	 ...
	 '_hookPull': function(channel)
	 {
	 	...
	 	this._janitor.addDisposable(this._virtualRouter.on('pull -stream ' + channel, function(request)
	 	{
	 		...
	 		send('pull', { hasNext: this.hasNext(), data: this.data } ).
	 		on(this).
	 		on({
	 			next: this.next,
	 		}).disposeOn('end');
	 ...
	 '_hookPush': function(channel)
	 {
	 	...
	 	this._janitor.addDisposable(this._virtualRouter.on('push -stream ' + channel, function()
	 	{
	 		...
	 		send('push', { hasNext: this.hasNext() }).
	 		on({
	 			next: this.next
	 		});
...
var LocalInvoker = Structr({
	...
	'hook': function(type, channel, ops)
	{
		...
		this._janitor.addDisposable(this._router.on(type + ' ' + channel, ops, function(localRequest)
		{
			...
			remoteMethod(channel, localRequest.data, { meta: { stream: true }, _next: _next }, function(remoteRequest)
			{
				if(type == 'pull')
				{
	...
	'push': function(trans)
	{
		this._router.push(' -stream ' + trans._name, {}, { from: this._con._remote, _next: this._next(trans) }, function(stream)
		{
	...
	'pull': function(trans)
	{
		...
		/*if(trans._name == 'some/random/callback')
		{
			console.log(this._router.getRoute('pull some/random/callback').listeners.length)
		}*/

		this._router.pull(' -stream ' + trans._name, trans._data.data, { from: this._con._remote, _next: this._next(trans) }, function()
		{
			this.pipe({
