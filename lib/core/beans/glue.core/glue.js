...
var GlueConnection = Structr({  
	...
	{    
		...
		{
			self._janitor.addDisposable(self._proxy.on('push -stream -glued -public ' + channel, function(stream)
			{ 
		...
		function listenToPull(channel)
		{   

			self._janitor.addDisposable(self._proxy.on('pull -stream -glued -public ' + channel, function(writer)
			{               
				if(!self.ignore(channel))
				{
		...
		function listenToChannel(channel)
		{
			...
			listenToPull(channel);
			listenOnPush(channel);
		}
