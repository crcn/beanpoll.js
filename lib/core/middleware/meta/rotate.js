...
exports.rotator = function(target, meta)
{
	...
	target.getRoute = function(ops)
	{
		...
		if(!ops.router._allowMultiple && route && route._meta && route._meta[meta] != undefined && listeners.length)
		{
			route._meta[meta] = (++route._meta[meta]) % listeners.length;

			//only ONE listener now..
			ops.listeners = [listeners[route._meta[meta]]];
		}
	}
